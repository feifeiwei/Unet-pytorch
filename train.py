...
test_loader = DataLoader(test, batch_size=6)
...
criteron = nn.BCELoss(reduction='elementwise_mean').to(device)
...
def test(epoch):
    ...
    for j, img in enumerate(test_loader):
        ...
        out = net(sr)
        loss = criteron(out, label)
        acc, pre,recall = eval_model(out, label)
    ...
    if total_loss < best_loss:
        ...
        torch.save(ckpt,'./weights/ckpt.pth')
